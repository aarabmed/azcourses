...
export default ({content,menu,title})=>{
    return <>
      <Head>
        <title>Udacity, {title} Free Premium Courses | AZcourses</title>
...
export const getServerSideProps = async ({query,params}) => {
    ...
    const category = platformsResponse.data.platforms.find(e=>e.platform==='udacity').categories.find(e=>e.slug===params.category)
    ...
    const courses_response = await axios.get(`process.env.udacityCourses.categories`)
